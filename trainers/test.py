...
def test(hparams, wandb_logger=None, model=None):
    ...
    if hparams.datatype == 'imaging' or hparams.datatype == 'multimodal':
        test_dataset = ImageDataset(hparams.data_test_eval_imaging, hparams.labels_test_eval_imaging, hparams.delete_segmentation,
                                    0, grab_arg_from_checkpoint(hparams, 'img_size'), target=hparams.target, train=False, live_loading=hparams.live_loading)
        ...
        if hparams.target == 'adoption':
            test_dataset = PetFinderConCatImageDataset(
                hparams.data_test_eval_tabular, hparams.data_test_eval_imaging)
