...
def load_datasets(hparams):
    if hparams.datatype == 'multimodal':
        ...
        if hparams.target == 'adoption':
            train_dataset = PetFinderConCatImageDataset(
                hparams.data_train_tabular, hparams.data_train_imaging)
            valid_dataset = PetFinderConCatImageDataset(
                hparams.data_val_tabular, hparams.data_val_imaging)
            hparams.input_size = train_dataset.__len__()
            return train_dataset, valid_dataset
    elif hparams.datatype == 'imaging':
        transform = grab_image_augmentations(
            hparams.img_size, hparams.target, hparams.crop_scale_lower)
        ...
        train_dataset = ContrastiveTabularDataset(
            hparams.data_train_tabular, hparams.labels_train, hparams.corruption_rate, hparams.field_lengths_tabular, hparams.one_hot)
        val_dataset = ContrastiveTabularDataset(
            hparams.data_val_tabular, hparams.labels_val, hparams.corruption_rate, hparams.field_lengths_tabular, hparams.one_hot)
        hparams.input_size = train_dataset.get_input_size()
    else:
...
def select_model(hparams, train_dataset):
    if hparams.datatype == 'multimodal':
        ...
        model = ImageModelPetFinderWithRTDL(
            n_num_features, cat_cardinalities, img_reduction_dim=40)
    elif hparams.datatype == 'imaging':
        if hparams.loss.lower() == 'byol':
            ...
            if not hparams.resume_training:
                model = SwAV(gpus=1, nmb_crops=(2, 0),
                             num_samples=len(train_dataset),  **hparams)
            else:
                model = SwAV(**hparams)
...
def pretrain(hparams, wandb_logger):
    ...
    logdir = create_logdir(
        hparams.datatype, hparams.resume_training, wandb_logger)
    ...
    if hparams.datatype != 'charms':
        if hparams.online_mlp:
            ...
            callbacks.append(SSLOnlineEvaluator(z_dim=model.pooled_dim, hidden_dim=hparams.embedding_dim,
                             num_classes=hparams.num_classes, swav=False, multimodal=(hparams.datatype == 'multimodal')))
        callbacks.append(ModelCheckpoint(
            filename='checkpoint_last_epoch_{epoch:02d}', dirpath=logdir, save_on_train_epoch_end=True, auto_insert_metric_name=False))
