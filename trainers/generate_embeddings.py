...
def generate_embeddings(hparams):
    ...
    if hparams.datatype == 'imaging' or hparams.datatype == 'multimodal':
        train_dataset = ImageDataset(hparams.data_train_eval_imaging, hparams.labels_train_eval_imaging,
                                     hparams.delete_segmentation, hparams.eval_train_augment_rate, hparams.img_size, train=False)
        ...
        train_dataset = TabularDataset(
            hparams.data_train_eval_tabular, hparams.labels_train_eval_tabular)
        val_dataset = TabularDataset(
            hparams.data_val_eval_tabular, hparams.labels_val_eval_tabular)
    ...
    for (loader, split) in [(train_loader, 'train'), (val_loader, 'val')]:
        ...
        save_path = os.path.join(grab_rundir_from_checkpoint(
            hparams.checkpoint), f'{split}_embeddings.pt')
