...
class ResnetEmbeddingModel(nn.Module):
    ...
    def __init__(self, args) -> None:
        ...
        self.projection_head = SimCLRProjectionHead(
            pooled_dim, original_args['embedding_dim'], original_args['projection_dim'])
        ...
        for k in list(state_dict.keys()):
            ...
            if k.startswith('projection_head_imaging.'):
                state_dict_projector[k[len(
                    'projection_head_imaging.'):]] = state_dict[k]

        log = self.backbone.load_state_dict(state_dict_encoder, strict=True)
        assert len(log.missing_keys) == 0
        log = self.projection_head.load_state_dict(
            state_dict_projector, strict=True)
        assert len(log.missing_keys) == 0
