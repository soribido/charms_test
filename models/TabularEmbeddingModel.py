...
class TabularEmbeddingModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        for k in list(state_dict.keys()):
            if k.startswith('encoder_projector_tabular.encoder.'):
                state_dict_encoder[k[len(
                    'encoder_projector_tabular.encoder.'):]] = state_dict[k]
            if k.startswith('encoder_projector_tabular.projector.'):
                state_dict_projector[k[len(
                    'encoder_projector_tabular.projector.'):]] = state_dict[k]
    ...
    def build_encoder(self, original_args: Dict) -> nn.Sequential:
        modules = [nn.Linear(original_args['input_size'],
                             original_args['embedding_dim'])]
        for _ in range(original_args['encoder_num_layers']-1):
            modules.extend([nn.ReLU(), nn.Linear(
                original_args['embedding_dim'], original_args['embedding_dim'])])
        return nn.Sequential(*modules)

    def build_projector(self, original_args: Dict) -> nn.Sequential:
        modules = [nn.ReLU(), nn.Linear(
            original_args['embedding_dim'], original_args['projection_dim'])]
        for _ in range(original_args['projector_num_layers']-1):
            modules.extend([nn.ReLU(), nn.Linear(
                original_args['projection_dim'], original_args['projection_dim'])])
        return nn.Sequential(*modules)
