...
class Pretraining(pl.LightningModule):
    ...
    def initialize_imaging_encoder_and_projector(self) -> None:
        ...
        self.projector_imaging = SimCLRProjectionHead(
            self.pooled_dim, self.hparams.embedding_dim, self.hparams.projection_dim)
    ...
    def initialize_tabular_encoder_and_projector(self) -> None:
        ...
        self.projector_tabular = SimCLRProjectionHead(
            self.hparams.embedding_dim, self.hparams.embedding_dim, self.hparams.projection_dim)
    ...
    def initialize_classifier_and_metrics(self, nclasses_train, nclasses_val):
        ...
        self.top1_acc_train = torchmetrics.Accuracy(
            task='multiclass', top_k=1, num_classes=nclasses_train)
        self.top1_acc_val = torchmetrics.Accuracy(
            task='multiclass', top_k=1, num_classes=nclasses_val)

        self.top5_acc_train = torchmetrics.Accuracy(
            task='multiclass', top_k=5, num_classes=nclasses_train)
        self.top5_acc_val = torchmetrics.Accuracy(
            task='multiclass', top_k=5, num_classes=nclasses_val)
        ...
        self.classifier_auc_train = torchmetrics.AUROC(
            task=task, num_classes=self.hparams.num_classes)
        self.classifier_auc_val = torchmetrics.AUROC(
            task=task, num_classes=self.hparams.num_classes)
    ...
    def load_pretrained_imaging_weights(self) -> None:
        ...
        _ = self.encoder_imaging.load_state_dict(
            state_dict_encoder, strict=True)
    ...
    def calc_and_log_train_embedding_acc(self, logits, labels, modality: str) -> None:
        ...
        self.log(f"{modality}.train.top1", self.top1_acc_train,
                 on_epoch=True, on_step=False)
        self.log(f"{modality}.train.top5", self.top5_acc_train,
                 on_epoch=True, on_step=False)
    ...
    def calc_and_log_val_embedding_acc(self, logits, labels, modality: str) -> None:
        ...
        self.log(f"{modality}.val.top1", self.top1_acc_val,
                 on_epoch=True, on_step=False)
        self.log(f"{modality}.val.top5", self.top5_acc_val,
                 on_epoch=True, on_step=False)
    ...
    def training_epoch_end(self, train_step_outputs: List[Any]) -> None:
        ...
        if self.current_epoch != 0 and self.current_epoch % self.hparams.classifier_freq == 0:
            ...
            self.estimator = LogisticRegression(
                class_weight='balanced', max_iter=1000).fit(embeddings, labels)
            ...
            self.log('classifier.train.accuracy',
                     self.classifier_acc_train, on_epoch=True, on_step=False)
            self.log('classifier.train.auc', self.classifier_auc_train,
                     on_epoch=True, on_step=False)
    ...
    def validation_epoch_end(self, validation_step_outputs: List[torch.Tensor]) -> None:
        ...
        if self.hparams.log_images:
            self.logger.log_image(key="Image Example", images=[
                                  validation_step_outputs[0]['sample_augmentation']])
        ...
        if not self.estimator is None and self.current_epoch % self.hparams.classifier_freq == 0:
            ...
            self.log('classifier.val.accuracy',
                     self.classifier_acc_val, on_epoch=True, on_step=False)
            self.log('classifier.val.auc', self.classifier_auc_val,
                     on_epoch=True, on_step=False)
    ...
    def stack_outputs(self, outputs: List[torch.Tensor]) -> torch.Tensor:
        ...
        for i in range(1, len(outputs)):
            ...
            embeddings = torch.cat(
                (embeddings, outputs[i]['embeddings']), dim=0)
