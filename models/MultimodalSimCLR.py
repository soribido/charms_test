...
class MultimodalSimCLR(Pretraining):
    ...
    def __init__(self, hparams):
        ...
        self.criterion_val = CLIPLoss(
            temperature=self.hparams.temperature, lambda_0=self.hparams.lambda_0)
        if self.hparams.loss.lower() == 'remove_fn':
            self.criterion_train = RemoveFNLoss(
                temperature=self.hparams.temperature, lambda_0=self.hparams.lambda_0)
        elif self.hparams.loss.lower() == 'binary_remove_fn':
            self.criterion_train = BinaryRemoveFNLoss(
                temperature=self.hparams.temperature, lambda_0=self.hparams.lambda_0)
        elif self.hparams.loss.lower() == 'supcon':
            self.criterion_train = SupConLossCLIP(
                temperature=self.hparams.temperature, lambda_0=self.hparams.lambda_0)
        elif self.hparams.loss.lower() == 'binary_supcon':
            self.criterion_train = BinarySupConCLIPLoss(
                temperature=self.hparams.temperature, lambda_0=self.hparams.lambda_0)
        elif self.hparams.loss.lower() == 'kpositive':
            self.criterion_train = KPositiveLossCLIP(
                temperature=self.hparams.temperature, k=6, cosine_similarity_matrix_path=self.hparams.train_similarity_matrix, threshold=self.hparams.threshold)
            ...
            raise ValueError(
                'The only implemented losses currently are CLIP, NTXent, supcon, and remove_fn')

        self.initialize_classifier_and_metrics(nclasses, nclasses)

        print(
            f'Tabular model, multimodal: {self.encoder_tabular}\n{self.projector_tabular}')
        print(
            f'Imaging model, multimodal: {self.encoder_imaging}\n{self.projector_imaging}')
    ...
    def training_step(self, batch: Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor, List[torch.Tensor]], _) -> torch.Tensor:
        ...
        if len(im_views[0]) == self.hparams.batch_size:
            self.calc_and_log_train_embedding_acc(
                logits=logits, labels=labels, modality='multimodal')
    ...
    def validation_step(self, batch: Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor, List[torch.Tensor]], _) -> torch.Tensor:
        ...
        if len(im_views[0]) == self.hparams.batch_size:
            self.calc_and_log_val_embedding_acc(
                logits=logits, labels=labels, modality='multimodal')
