...
class ImagingModel(nn.Module):
    ...
    def __init__(self, args) -> None:
        ...
        if args.checkpoint:
            ...
            if 'encoder_imaging.0.weight' in state_dict:
                ...
                encoder_name_dict = {'clip': 'encoder_imaging.', 'remove_fn': 'encoder_imaging.', 'supcon': 'encoder_imaging.',
                                     'byol': 'online_network.encoder.', 'simsiam': 'online_network.encoder.', 'swav': 'model.', 'barlowtwins': 'network.encoder.'}
            ...
            for k in list(state_dict.keys()):
                if k.startswith(self.encoder_name) and not 'projection_head' in k and not 'prototypes' in k:
                    state_dict_encoder[k[len(
                        self.encoder_name):]] = state_dict[k]
    ...
    def create_imaging_model(self, args):
        if args['model'] == 'resnet18':
            ...
            raise Exception(
                'Invalid architecture. Please select either resnet18 or resnet50.')
