...
class Evaluator_Regression(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.pcc_train = torchmetrics.PearsonCorrCoef(
            num_outputs=hparams.num_classes)
        self.pcc_val = torchmetrics.PearsonCorrCoef(
            num_outputs=hparams.num_classes)
        self.pcc_test = torchmetrics.PearsonCorrCoef(
            num_outputs=hparams.num_classes)
    ...
    def test_epoch_end(self, _) -> None:
        ...
        self.log('test.pcc.mean', test_pcc_mean,
                 metric_attribute=self.pcc_test)
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor], _) -> torch.Tensor:
        ...
        self.log('eval.train.mae', self.mae_train,
    ...
    def training_epoch_end(self, _) -> None:
        ...
        self.log('eval.train.pcc.mean', epoch_pcc_train_mean,
                 on_epoch=True, on_step=False, metric_attribute=self.pcc_train)
    ...
    def validation_epoch_end(self, _) -> None:
        ...
        self.log('eval.val.mae', epoch_mae_val, on_epoch=True,
                 on_step=False, metric_attribute=self.mae_val)
        self.log('eval.val.pcc.mean', epoch_pcc_val_mean, on_epoch=True,
                 on_step=False, metric_attribute=self.pcc_val)
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=int(
            10/self.hparams.check_val_every_n_epoch), min_lr=self.hparams.lr*0.0001)
        return optimizer
