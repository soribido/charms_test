...
class ContrastiveImagingAndTabularDataset(Dataset):
    ...
    def corrupt(self, subject: List[float]) -> List[float]:
        ...
        indices = random.sample(
            list(range(len(subject))), int(len(subject)*self.c))
        for i in indices:
        ...
        for i in range(len(subject)):
            if self.field_lengths_tabular[i] == 1:
                ...
                out.append(torch.nn.functional.one_hot(
                    subject[i].long(), num_classes=int(self.field_lengths_tabular[i])))
    ...
    def __getitem__(self, index: int) -> Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        tabular_views = [torch.tensor(self.data_tabular[index], dtype=torch.float), torch.tensor(
            self.corrupt(self.data_tabular[index]), dtype=torch.float)]
        if self.one_hot_tabular:
...
class ContrastiveImagingAndTabularDataset_PetFinder(ContrastiveImagingAndTabularDataset):
    ...
    def __init__(
        ...
        # Tabular
        self.c = corruption_rate
        self.field_lengths_tabular = torch.tensor(
            [2, 1, 20, 20, 3, 7, 7, 6, 4, 3, 3, 3, 3, 3, 1, 1, 14, 1, 1, 1, 1, 1, 1, 1], dtype=torch.int).cpu()
        self.one_hot_tabular = one_hot_tabular
        self.data_tabular = self.read_and_parse_csv(data_path_tabular)
        ...
        self.data_tabular.drop(
            columns=['AdoptionSpeed', 'PetID'], inplace=True)
        self.generate_marginal_distributions(data_path_tabular)
    ...
    def __getitem__(self, index: int) -> Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor]:
        imaging_views, unaugmented_image = self.generate_imaging_views(
            index=index)
