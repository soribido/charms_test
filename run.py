...
def run(args: DictConfig):
    ...
    # Prevents multiple runs getting the same version when launching many jobs at once
    ...
    if args.use_wandb:
        if args.resume_training and args.wandb_id:
            wandb_logger = WandbLogger(project=args.wandb_project, entity=args.wandb_entity,
                                       save_dir=base_dir, offline=args.offline, id=args.wandb_id, resume='must')
        else:
            wandb_logger = WandbLogger(project=args.wandb_project, entity=args.wandb_entity,
                                       save_dir=base_dir, offline=args.offline, log_model='all')
    else:
        wandb_logger = WandbLogger(
            name=args.target, project='Test', entity='', save_dir=base_dir, offline=args.offline)
    ...
    if args.pretrain:
        ...
        args.checkpoint = os.path.join(
            base_dir, 'runs', args.datatype, wandb_logger.experiment.name, f'checkpoint_last_epoch_{args.max_epochs-1:02}.ckpt')
